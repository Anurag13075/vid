...
// In production, serve the built frontend and let the SPA handle routing
if (IS_PROD) {
...
async function main() {
  try {
    await initDb();
    app.listen(PORT, () => {
      console.log(`🚀 VidRush server running on http://localhost:${PORT} [${IS_PROD ? "production" : "development"}]`);
    });
  } catch (err) {
