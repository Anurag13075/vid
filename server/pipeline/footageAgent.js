...
async function searchPexels(query: string, usedIds: Set<string>): Promise<Clip | null> {
  ...
  try {
    ...
    for (const v of data.videos || []) {
      ...
      // Prefer HD (1280–1920) — avoid 4K clips which are huge and slow to download
      const hd = v.video_files
        .filter((f) => f.file_type === "video/mp4" && f.width >= 1280 && f.width <= 1920)
        .sort((a, b) => b.width - a.width)[0];
      const best = hd ?? v.video_files
        .filter((f) => f.file_type === "video/mp4" && f.width >= 1280)
        .sort((a, b) => a.width - b.width)[0]; // pick smallest 4K if no HD
      if (!best) continue;
      return {
        id: v.id,
        keyword: query,
...
// ── Downloader ────────────────────────────────────────────────────────────────
// Hard 25s wall-clock budget (headers + body). Uses Node.js stream pipeline so
// the AbortController properly kills the body read, not just the header fetch.
export async function downloadClip(
  ...
  // AbortController kills the fetch (including body stream) on timeout
  const controller = new AbortController();

  // Hard 25s wall-clock budget for the entire download
  ...
  try {
    ...
    // Stream body directly to disk — avoids buffering the whole clip in RAM
    // and properly propagates the AbortController signal to the read loop.
